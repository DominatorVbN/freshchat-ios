...
+(void)sendLatestUserActivity:(FCChannels *)channel{
    ...
    NSPredicate *queryType = [NSPredicate predicateWithFormat:@"isWelcomeMessage == 0 AND messageUserAlias != %@ AND messageType < 1000", USER_TYPE_MOBILE];
...
+(NSURLSessionDataTask *)restoreUserWithJwtToken:(NSString *)jwtIdToken withCompletion:(void (^)(NSError *))completion{
    ...
    NSURLSessionDataTask *task = [apiClient request:request isIdAuthEnabled:YES withHandler:^(FCResponseInfo *responseInfo, NSError *error) {
        ...
        if([[FCReachabilityManager sharedInstance] isReachable] && statusCode) {
